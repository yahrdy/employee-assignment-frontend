  ...
  <v-container>
    ...
    >
      ...
      <template v-slot:item.name="{ item }">
        <router-link :to="{name: 'ManageDepartments',params:{companyId: item.id}}">
          {{ item.name }}
        </router-link>
      </template>
...
export default {
  ...
  methods: {
    initialize() {
      this.loading = true
      const url = 'companies?page=' + this.options.page + '&per_page=' + this.options.itemsPerPage + this.sortTerm + this.descTerm + this.searchTerm
    ...
    deleteItemConfirm() {
      const url = 'companies/' + this.editedItem.id
      let editedIndex = this.editedIndex
      axios.delete(url).then(() => {
        this.companies.splice(editedIndex, 1)
        this.closeDelete()
        this.color = 'red'
        this.snackbarText = 'Company deleted'
        this.snackbar = true
      })
    },
    ...
    save() {
      if (this.editedIndex > -1) {
        ...
        axios.post(url, this.form).then((response) => {
          Object.assign(this.companies[editedIndex], response.data)
          this.close()
          this.color = 'green'
          this.snackbarText = 'Company updated'
          this.snackbar = true
          this.busy = false
        }).catch((error) => {
        ...
        axios.post(url, this.form).then((response) => {
          ...
          this.close()
          this.color = 'green'
          this.snackbarText = 'New company created'
          this.snackbar = true
          this.busy = false
        }).catch((error) => {
          this.errorMessages = error.response.data
